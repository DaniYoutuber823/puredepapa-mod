...
class PlayState extends MusicBeatState
{
	...
	public var BfCorruptedLines:Array<String> = ["ESTO NO ESTA BIEN", "AAAAAAAAAA", "NO RESPONDE", "QUE ME PASA", "DONDE ESTOY", "AYUDA", "ALT F4", "NOOOOOO"];

	public var originalX:Float;

	public static var dad:Character;
	public static var dadFlash:Character;
	...
	var bstatic:FlxSprite = new FlxSprite(0,0).loadGraphic(Paths.image('BfStatic','clown'), true, 320, 180);
	...
	override public function create()
	{
					...
					if(SONG.song.toLowerCase() == 'revenge')
						{
							...
							bgFlash = new FlxSprite(posX, posY);
							bgFlash.frames = Paths.getSparrowAtlas('weeb/animatedEvilSchool','week6');
		...
		if(FlxG.save.data.character == 1){
			curBf = 'bf-pixel-corrupted';
		}else if(FlxG.save.data.character == 2) {
			curBf = 'carne-pixel-corrupted';
		}else {
			curBf = SONG.player1;
		}

		
		boyfriend = new Boyfriend(770, 450, curBf);
		...
		add(dad);
		if(SONG.song.toLowerCase() == 'revenge'){
			dadFlash = new Character(100, 100, 'spirit');
		...
		if (curStage == 'auditorHell')
			{
				...
				cloneOne.frames = Paths.getSparrowAtlas('fourth/Clone','clown');
				cloneTwo.frames = Paths.getSparrowAtlas('fourth/Clone','clown');
				cloneOne.alpha = 0;
				cloneTwo.alpha = 0;
		...
		iconP1 = new HealthIcon(curBf, true);
	...
	function doStopSign(sign:Int = 0, fuck:Bool = false)
		{
			...
			// Caching.images.get('sign')
	
			daSign.frames = Paths.getSparrowAtlas('fourth/mech/Sign_Post_Mechanic','clown');
	
			daSign.setGraphicSize(Std.int(!FlxG.save.data.bigsign? daSign.width * 0.67 : daSign.width * 1.5));
	...
	function startSong():Void
	{
		...
		}else if(curBf == 'carne-pixel-corrupted'){
			FlxTween.tween(boyfriend, {
				x: boyfriend.x,
				y: boyfriend.y - 320
			}, 2, {
		...
		if(SONG.player2 == 'bf-pixel-corrupted'){
			FlxTween.tween(dad, {
				x: dad.x,
				y: dad.y - 320
			}, 2, {
	...
	override public function update(elapsed:Float)
	{
		...
		if(SONG.song.toLowerCase() == 'revenge'){
			if(glitching)
			{
				dad.alpha = dad.alpha != 1 ? 1 : 0;
				
				dadFlash.alpha = dadFlash.alpha != 1 ? 1 : 0;

				bgFlash.alpha = bgFlash.alpha != 1 ? 1 : 0;
				
				/*bgSky.alpha = bgSky.alpha != 1 ? 1 : 0;
				
				bgSchool.alpha = bgSchool.alpha != 1 ? 1 : 0;
				
				bgStreet.alpha = bgStreet.alpha != 1 ? 1 : 0;
				
				fgTrees.alpha = fgTrees.alpha != 1 ? 1 : 0;
				
				bgTrees.alpha = bgTrees.alpha != 1 ? 1 : 0;

				bgGirls.alpha = bgTrees.alpha != 1 ? 1 : 0;
				
				treeLeaves.alpha = treeLeaves.alpha != 1 ? 1 : 0;*/
		...
		{
			FlxG.switchState(new AnimationDebug(curBf));
			#if windows
			if (luaModchart != null)
			{
				luaModchart.die();
							...
							if(!daNote.burning){
								switch (Math.abs(daNote.noteData))
								{
									case 2:
										...
										if(SONG.song.toLowerCase() == 'revenge'){
											dadFlash.playAnim('singUP' + altAnim, true);
										}
									case 3:
										dad.playAnim('singRIGHT' + altAnim, true);
										if(SONG.song.toLowerCase() == 'revenge'){
											dadFlash.playAnim('singRIGHT' + altAnim, true);
										}
									case 1:
										dad.playAnim('singDOWN' + altAnim, true);
										if(SONG.song.toLowerCase() == 'revenge'){
											dadFlash.playAnim('singDOWN' + altAnim, true);
										}
									case 0:
										dad.playAnim('singLEFT' + altAnim, true);
										if(SONG.song.toLowerCase() == 'revenge'){
											dadFlash.playAnim('singLEFT' + altAnim, true);
										}
								}
							}
						}
							...
							switch(dad.curCharacter)
							{
								case 'exTricky': // 60% chance
									if (FlxG.random.bool(60) && !spookyRendered && !daNote.isSustainNote) // create spooky text :flushed: (😳)
										{
											createSpookyText(ExTrickyLinesSing[FlxG.random.int(0,ExTrickyLinesSing.length)], false);
										}
							}
						}
	...
	function createSpookyText(text:String, isBf:Bool, x:Float = -1111111111111, y:Float = -1111111111111):Void
		{
			spookySteps = curStep;
			spookyRendered = true;
			tstatic.alpha = 0.5;
			...
			if(isBf){
				...
				spookyText.setFormat("Impact", 128, 0xFF00FFFB);
			}else{
								...
								if (coolNote.burning)
									{
										...
										else
										{
											...
											coolNote.destroy();
											FlxG.sound.play(Paths.sound('burnSound','week5'));
											playerStrums.forEach(function(spr:FlxSprite)
											{
												if (pressArray[spr.ID] && spr.ID == coolNote.noteData)
												{
													...
													smoke.frames = Paths.getSparrowAtlas('Smoke','week5');
	...
	*/
		...
		function goodNoteHit(note:Note, resetMashViolation = true):Void
			{
				...
				if (!note.wasGoodHit)
				{
					...
					switch(boyfriend.curCharacter){
						case 'bf-pixel-corrupted':
							if (FlxG.random.bool(30) && !spookyRendered && !note.isSustainNote) // create spooky text :flushed:
								{
	...
	override function stepHit()
	{
		...
		{
			case 'revenge':
				if(curStep == 844 || curStep == 845 || curStep == 846 || curStep == 847 || curStep == 860 || curStep == 861 || curStep == 862 || curStep == 863 || curStep == 925 || curStep == 926 || curStep == 927 || curStep == 972 || curStep == 973 || curStep == 974 || curStep == 975 || curStep == 988 || curStep == 989 || curStep == 990 || curStep == 991 || curStep == 1164 || curStep == 1165 || curStep == 1166 || curStep == 1167 || curStep == 1181 || curStep == 1182 || curStep == 1183 || curStep == 1228 || curStep == 1229 || curStep == 1230 || curStep == 1231 || curStep == 1244 || curStep == 1245 || curStep == 1246 || curStep == 1247 || curStep == 1356 || curStep == 1357 || curStep == 1358 || curStep == 1359 || curStep == 1372 || curStep == 1373 || curStep == 1374 || curStep == 1375)
				{
					glitching = true;
				}
				else
