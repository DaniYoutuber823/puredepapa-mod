...
class BigSignOption extends Option
{
	public function new(desc:String)
	{
		super();
	...
	public override function press():Bool
	{
		FlxG.save.data.bigsign = !FlxG.save.data.bigsign;
		display = updateDisplay();
		return true;
	}
	...
	private override function updateDisplay():String
	{
		return "Big Signs " + (!FlxG.save.data.bigsign ? "off" : "morir");
	}
}
...
{
	var curCharacter:String = 'Boyfriend';
	public function new(desc:String)
	{
		super();
		description = desc;
	...
	override function right():Bool {
		if (FlxG.save.data.character > 2)
		{
			FlxG.save.data.character = 1;
		}
		else
			FlxG.save.data.character = FlxG.save.data.character + 1;

		return true;
	}

	override function left():Bool {
		if (FlxG.save.data.character <= 0)
			{
				FlxG.save.data.character = 2;
			}
		else
			FlxG.save.data.character = FlxG.save.data.character - 1;
		return true;
	...
	override function getValue():String
	{
		if(FlxG.save.data.character == 1){
			curCharacter = 'Corrupted Boyfriend';
		}else if(FlxG.save.data.character == 2){
			curCharacter = 'Corrupted Senpai';
		}else{
			curCharacter = 'Boyfriend';
		}
		return "Current Character: " + curCharacter;
	}
}
...
class CustomizeCharacter extends Option
{
	public function new(desc:String)
	{
		super();
	...
	{
		trace("switch");
		FlxG.switchState(new CharacterCustomizeState());
		return false;
	}

	private override function updateDisplay():String
	{
		return "Customize Skin";
	}
}
